...
局域网文件/文件夹传输插件:让 DeepSeek Harness 通过对话,把文件(或**自动打包成 zip 的文件夹**)发送到同一局域网内运行 [LocalSend](https://github.com/localsend/localsend) 的机器。
...
- **GUI 配置卡片**:「设置 → 插件配置」中的「LocalSend 局域网传输」卡片,可编辑别名/端口/各类超时/单文件上限,保存即生效(新会话的工具调用生效)。
- **健壮性**:别名重名/模糊匹配有提示;错误码(403 拒绝、409 繁忙、429 限流、422 校验失败等)映射为可读文案;失败自动取消会话;跳过符号链接与空目录;单文件/单包上限默认 1 GiB(可配置)。
- **安全默认**:仅局域网;扫描只读;发送方别名默认主机名;接收端人工“接受”即授权闸门;自签 HTTPS + sha256 端到端校验。
...
`#v0.2.0` 固定到该发布版本;去掉后缀则跟随 `main` 分支最新提交。安装后**重启 DSH**:新会话出现两个工具,GUI 刷新后「设置 → 插件配置」出现卡片。
